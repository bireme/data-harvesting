...
@task
def get_collections_to_be_transformed():
    logger = logging.getLogger(__name__)
    mongo_hook = MongoHook(mongo_conn_id='mongo')
    mongo_db = 'OJS'
...
def transform_ojs_data(coll_name, id_offset):
    logger = logging.getLogger(__name__)
    mongo_hook = MongoHook(mongo_conn_id='mongo')
    mongo_db = 'OJS'
    
    logger.info(f"Processando coleção: {coll_name} com offset de ID: {id_offset}")
    ...
    editor_cc_code = journal_data.get('editor_cc_code')
    if editor_cc_code:
        editor_cc_code = editor_cc_code.strip()

    if not journal_title:
        logger.warning(f"Journal title missing for {journal_id}. Aborting task.")
        return
    ...
    for record in records:
        ...
        if check_duplicate_ojs_id(mongo_hook, record['id']):
            logger.info(f"Duplicate OJS ID found in id_ojs: {record['id']}. Skipping.")
            continue
        ...
        if check_duplicate_nao_fiadmin(mongo_hook, record['id']):
            logger.info(f"Duplicate OJS ID found in ojs_nao_fi-admin: {record['id']}. Skipping.")
            continue
...
with DAG(
    ...
    collections_to_be_transformed = get_collections_to_be_transformed()

    transform_ojs_data_task = PythonOperator.partial(
        task_id='transform_ojs_data',
        python_callable=transform_ojs_data
    ).expand(op_kwargs=collections_to_be_transformed)

    setup_ojs_transformed_task >> collections_to_be_transformed >> transform_ojs_data_task